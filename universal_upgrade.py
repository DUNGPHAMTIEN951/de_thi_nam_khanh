...
for filename in os.listdir(directory):
    if filename.endswith(".html") and filename.startswith("de_"):
        ...
        # 1. Extract database
        db_match = re.search(r'const database = (\{.*?\});', content, re.DOTALL)
        if not db_match:
            # Try another pattern for database
            db_match = re.search(r'const database = (\{.*)', content, re.DOTALL)
            if db_match:
                # Find the end of JSON by matching braces
                json_str = ""
                brace_count = 0
                for char in db_match.group(1):
                    json_str += char
                    if char == '{': brace_count += 1
                    elif char == '}': brace_count -= 1
                    if brace_count == 0: break
                database_str = json_str
            else:
                print(f"Database not found in {filename}")
                continue
        else:
            database_str = db_match.group(1)

        # 2. Extract Test ID
        id_match = re.search(r'currentTestId = "(.*?)";', content)
        if not id_match:
        ...
        new_script = f"const database = {database_str};\n" + universal_script_template.replace("{TEST_ID}", test_id)
        
        # 4. Replace entire <script> block
        new_content = re.sub(r'<script>.*?</script>', f'<script>{new_script}</script>', content, flags=re.DOTALL)
        
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print(f"Universal Upgrade applied to {filename}")
