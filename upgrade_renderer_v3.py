...
new_renderer_js = """
        ...
        function renderQuestions(containerId, data) {
            ...
            while (i < data.length) {
                ...
                if (q.text.includes('(1)') || q.text.includes('(1-') || q.text.includes('(1 –')) {
                    ...
                    while (j < data.length) {
                        ...
                        if (numMatch || (j === i)) {
                            ...
                            // Extract answer for Word Box if it's a simple fill-in
                            if (nextQ.answer) {
                                let ans = Array.isArray(nextQ.answer) ? nextQ.answer[0] : nextQ.answer;
                                if (ans.length < 20) wordBox.push(ans);
                            }
                ...
                // 2. READING PASSAGE DETECTION (Very long text)
...
for filename in os.listdir(directory):
    if filename.endswith(".html") and filename.startswith("de_"):
        ...
        script_pattern = r'function renderQuestions\(containerId, data\) \{.*?\}\n\n\s+function initApp\(\)'
        if re.search(script_pattern, content, re.DOTALL):
            ...
            # Remove redundant ml-14 in gradeTest to fit new layout
            new_content = new_content.replace('shadow-sm ml-14"', 'shadow-sm"')
            
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(new_content)
            print(f"Upgraded {filename} to V3.0 (Smart Renderer)")
        else:
